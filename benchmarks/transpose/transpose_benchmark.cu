...
int transpose_benchmark(std::size_t n) {
    ...
    const bool naive_valid = run_and_validate_transpose(
        hpc::TransposeAlgo::Naive, device_a, device_b, gpu_result.data(),
        cpu_result.data(), n);

    const bool shared_valid = run_and_validate_transpose(
        hpc::TransposeAlgo::Shared, device_a, device_b, gpu_result.data(),
        cpu_result.data(), n);

    const bool padding_valid = run_and_validate_transpose(
        hpc::TransposeAlgo::Padding, device_a, device_b, gpu_result.data(),
        cpu_result.data(), n);

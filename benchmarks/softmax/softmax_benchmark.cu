...
bool run_and_validate_softmax(hpc::SoftmaxAlgo algo,
    ...
    hpc::softmax(device_input.data(), device_output.data(), rows, cols, algo);
...
int softmax_benchmark(std::size_t rows, std::size_t cols) {
    ...
    cuda_bench::DeviceBuffer<float> device_output(size);
    device_input.copy_from_host(input.data());

    const bool naive_valid = run_and_validate_softmax(
        hpc::SoftmaxAlgo::Naive, device_input, device_output, gpu_result.data(),
        cpu_result.data(), rows, cols);

    const bool shared_memory_valid = run_and_validate_softmax(
        hpc::SoftmaxAlgo::SharedMemory, device_input, device_output,
